    ...
    ui.div(
        ...
        ui.navset_tab(
            ui.nav_panel(
                ...
                # ── Top Movers + Revenue by Segment side by side ──────────────
                ui.div(
                    ui.div(
                ...
                # ── Revenue-by-market trend: own full-width row (needs room) ───
                ui.div(
...
def server(input, output, session):
    ...
    def _kpi_card(icon, icon_class, label, value, delta_pct, sub_text, tooltip):
        ...
        return ui.tags.div(
            ...
            class_="metric-card",
            style="flex: 1 1 200px; min-width: 165px;"
        )
    ...
    def _hero_cards():
        """The 4-card hero band: 营业额 · 净贡献 · 成单数 · 客单价."""
        df = filtered_data(); prev = previous_period_data()
        currency = currency_converter(); rate, sym = currency['rate'], currency['symbol']
        nc = net_contribution_calc()
